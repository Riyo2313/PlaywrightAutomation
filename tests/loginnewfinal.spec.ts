...
await expect(confmesg).toHaveText('Deposit successful.', {timeout:2000})
 await page.screenshot({ path: 'depositsuccess.png' });
...
const withdrawAmounts=['700', '700', '1500','-400']
...
const withdrawmsg= page.locator('p#withdrawMsg.success')
  await expect(withdrawmsg).toHaveText('Withdrawal successful.',{timeout:2000})
...
await page.screenshot({ path: 'after-error.png' });
  // Example assertion (customize based on your app's behavior)
  // e.g., check if dashboard is visible
...
await page.screenshot({ path: 'after-console2' });
